...
def build_monthly_audit(year: int, month: int, selected_day: int, shift: str = "night", internal_state: dict = None):
    ...
    # 1. Извлечение инцидентов из Django ORM
    ...
    for inc in sel_incidents:
        ...
        if inc.device_type == 'sw' or (':' not in target and inc.device_type != 'olt'):
            info = sw_map.get(target, {"location": "Коммутаторы на ONU", "desc": "—"})
            loc = info["location"]
            sw_groups_raw.setdefault(loc, {}).setdefault(target, {"contract": info["desc"], "events": []})
            ...
            gpon_olts_raw.setdefault(olt_ip, {}).setdefault(port_name, {})[onu_id] = inc
    ...
    for olt_ip, ports_dict in gpon_olts_raw.items():
        ...
        for port_name, onus_dict in ports_dict.items():
            onus_list = []
            for onu_id, inc in onus_dict.items():
                olt_los += 1
                dur = (inc.end_time or now) - inc.start_time
                onus_list.append({
                    "id": onu_id,
                    "contract": "—",
                    "state": "LOS",
                    "time_start": datetime.fromtimestamp(inc.start_time).strftime("%H:%M"),
                    "time_end": datetime.fromtimestamp(inc.end_time).strftime("%H:%M") if inc.end_time else "Активен",
                    "duration_str": format_sec_to_str(dur)
                })
